...
public class AdminController {
	...
	public String board_write(RedirectAttributes rdat,MultipartFile file, BoardVO boardVO) throws Exception {
		...
		if(file.getOriginalFilename() != "") {// 첨부파일이 있으면
			...
			String[] real_file_names = new String[] {file.getOriginalFilename()}; // "한글파일명.jsp"
	...
	public String board_list(@ModelAttribute("pageVO") PageVO pageVO, Model model) throws Exception {
		...
		// PageVO의 queryStartNo구하는 계산식이 먼저 실행되어서 변수값이 발생되야함.
		if(pageVO.getPage() == null) {//int 일때 null체크에러가 나와서 pageVO의 page변수형 Integer로벼경.
			pageVO.setPage(1);
		}
		pageVO.setPerPageNum(8);//리스트하단에 보이는 페이징번호의 개수
		pageVO.setQueryPerPageNum(10);//쿼리에서 1페이지당 보여줄 게시글수 10명으로 입력 놓았습니다.
		//검색된 전체 회원 개시글 개수 구하기 서비스 호출
		int countBoard = 0;
		countBoard = boardService.countBoard(pageVO);
		pageVO.setTotalCount(countBoard);//전체 게시글의 수를 구한 변수 값 매개변수로 입력하는 순간 calcPage()메서드실행.
